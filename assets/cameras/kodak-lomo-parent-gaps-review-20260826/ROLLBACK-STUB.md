...
`public/assets/cameras/kodak-lomo-parent-gaps-review-20260826/`

No remote or App restoration is required. Rejected references remain under `rejected/` until the user completes review.

After approval only, upload selected finals to `RollNote-assets`, use their GitHub Pages URLs for variant-specific App mappings, preserve each current mapping in `imageStub`, and record the remote asset commit plus exact rollback instructions before integration.
